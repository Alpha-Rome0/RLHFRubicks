...
def get_prompt(scramble):
    ...
    return f"""You are a Rubik's cube solving assistant. Your job is to generate the space separated moves needed to solve a Rubik's cube when given the Rubik's cube scramble. A scramble is a list of moves that are performed on a fully solved Rubik's cube in order to scramble it up.
